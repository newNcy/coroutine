...
void event_init(io_mgr_t * io_mgr)
{
    ...
    state->max = 1024;
    state->fdd = epoll_create(1);
    state->events = (struct epoll_event*)malloc(state->epoll_max * sizeof(struct epoll_event));
...
void event_add(io_mgr_t * io_mgr, int fd)
{
    ...
    epoll_ctl(state->fdd, EPOLL_CTL_ADD, fd, &event);
}

void event_del(io_mgr_t * io_mgr_t, int fd)
{
    epoll_state_t * state = (epoll_state_t*)io_mgr->event_ctx;
    epoll_ctl(state->fdd, EPOLL_CTL_DEL, fd, NULL);
}

int event_wait(io_mgr_t * io_mgr_t, long long timeout)
{
    epoll_state_t * state = (epoll_state_t*)io_mgr->event_ctx;
    int ready = epoll_wait(state->fd, state->events, state->max, timeout);
    array_resize(&io_mgr->fired_events, ready);
    for (int i = 0; i < ready; ++ i) {
        struct epoll_event * e = state->events[i].events; 
        int events = 0;
        if (e->events & EPOLLIN) { events |= IO_READ;}
        if (e->events & EPOLLOUT) { events |= IO_WRITE;}
        array_set(&io_mgr->fired_events, i, encode_event(e->data.fd, type));
    }
    return ready;
}

void event_destroy(io_mgr_t * io_mgr_t)
{
}
